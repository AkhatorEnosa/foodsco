...
const TestimonialCard = ({ text, name, image, title, variant }) => {
  return (
    <div className="flex flex-col border-[1px] border-[#E3EBF6] text-sm lg:text-base rounded-3xl bg-white p-6 justify-between gap-10 z-20">
        ...
        <div className="flex flex-col gap-[33px]">
            ...
            <div className="flex gap-6 items-center">
                <div className="w-[50px] h-[50px] lg:w-fit lg:h-fit rounded-full overflow-clip">
                    <img src={image} alt="person-img" />
                </div>
                <div>
                    <h3 className="person-name font-medium text-lg md:text-2xl">{name}</h3>
                    <p>{title}</p>
                </div>
            </div>
        </div>
